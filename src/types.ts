...
export interface ExamPattern {
  ...
  columns: ExamPatternColumn[];
  rows: ExamPatternRow[];
  cells: ExamPatternCell[];
}
...
export interface EligibilityResult {
  overallStatus: 'ELIGIBLE' | 'CHECK_REQUIREMENTS' | 'NOT_ELIGIBLE';
  badgeColor: 'GREEN' | 'YELLOW' | 'RED';
  criteriaResults: { name: string; status: 'PASS' | 'FAIL' | 'REVIEW'; detail: string }[];
...
export type WeeklyPlanItem = RoadmapWeekPlan;
...
export interface ResumeJobMatchItem {
  job: Job;
  matchScore: number;
...
export interface ResumeSkillGapAnalysis {
  candidateQualification: string;
  candidateSkills: string[];
  evaluatedVacanciesCount: number;
  criticalMissingSkills: string[];
  recommendedPreparationPath: {
    phase: string;
    focus: string;
    milestone: string;
  }[];
